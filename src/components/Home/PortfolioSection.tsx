      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8  flex flex-col gap-4 text-center items-center justify-center">
        ...
        <motion.div
          ...
          {sectors.map((item, index) => (
            <Link
              href={item.link as Parameters<typeof Link>[0]["href"]}
              key={index}
              className="block">
              <motion.div
                className={`w-80 lg:w-[20rem] h-[20rem] md:h-[22rem] lg:h-[25rem] flex flex-col gap-2 bg-black/3 p-6 overflow-hidden relative cursor-pointer ${
                  index === 1
                    ? "lg:mt-10"
                    : index === 2
                    ? "lg:-mt-10"
