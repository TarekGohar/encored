...
export async function generateMetadata({
  ...
  return {
    title: "Elisa Cavaletti",
    description: t("elisa-description"),
  };
}
...
//   const imagesDir = path.join(process.cwd(), "public/images/elisa-cavaletti");
//   const files = fs
...
//       }
//       return {
//         src: `/images/elisa-cavaletti/${file}`,
    ...
    <>
      <div>
        ...
        <div className="h-[20rem] md:h-[36rem] flex justify-center md:justify-start items-center">
          <div className="md:container">
            <Image
              src="/images/elisa-logo.webp"
              width={1000}
              height={1000}
              alt="Ecla logo"
              className="w-[22rem] md:w-[34rem] opacity-90 px-4"
            />
          </div>
        </div>
      </div>
      <div className="bg-neutral-100">
        <div className="container h-fit py-12 mb-2 md:mb-4 flex flex-col items-center justify-center px-4 gap-y-4">
          ...
          <a
            href="https://elisacavaletti.it/en/"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2 md:gap-4">
          {images.map(({ src, width, height, isHorizontal }, index) => (
            <Image
              key={index}
              src={src}
              alt={`Ecla Image ${index + 1}`}
              width={isHorizontal ? width : 2000} // Adjust width for better grid layout
